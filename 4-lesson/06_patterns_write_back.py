...
# Очередь для отложенной записи в БД
...
async def save_to_database(item_id: int, data: dict):
    ...
    await asyncio.sleep(2)  # Имитация задержки БД
    fake_db[item_id] = data
    return True

async def flush_write_queue():
    """Фоновая задача для записи данных из очереди в БД"""
    while True:
        await asyncio.sleep(5)  # Записываем в БД каждые 5 секунд
...
async def write_to_db_delayed(item_id: int, data: dict):
    await asyncio.sleep(3)  # Задержка перед записью
    await save_to_database(item_id, data)
...
async def create_or_update_item(item_id: int, item: Item):
    ...
    1. Записываем данные ТОЛЬКО в кэш (быстро)
    2. Возвращаем успешный ответ пользователю
    3. Запись в БД происходит асинхронно в фоне
    ...
    - Кэш и БД временно не синхронизированы
    """
...
async def read_item(item_id: int):
    """
    Чтение данных:
    1. Сначала проверяем кэш (самые свежие данные)
